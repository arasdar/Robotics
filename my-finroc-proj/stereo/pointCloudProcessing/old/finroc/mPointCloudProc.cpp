...
//
//----------------------------------------------------------------------
/*!\file    projects/icarus/sensor_processing/pointCloudProcessing/StereoVision/mPointCloudProc.cpp
 *
...
// Const values
//----------------------------------------------------------------------
runtime_construction::tStandardCreateModuleAction<mPointCloudProc> cCREATE_ACTION_FOR_M_POINTCLOUDPROC("PointCloudProc");
...
//----------------------------------------------------------------------
// mPointCloudProc constructor
//----------------------------------------------------------------------
mPointCloudProc::mPointCloudProc(core::tFrameworkElement *parent, const std::string &name) :
  ...
  prev_ground_image_test(new Cloud) //ransac
{
  ...
  viewer->registerKeyboardCallback(&mPointCloudProc::keyboardCallback, *this, 0);


  // Set up a 3D viewer
  viewer_test->setBackgroundColor(0, 0, 0);
  viewer_test->setCameraPosition(0, 0, -2, 0, -1, 0, 0);  //(0, 0, -2, 0, -1, 0, 0)
  viewer_test->registerKeyboardCallback(&mPointCloudProc::keyboardCallback, *this, 0);
  ...
  /* reading images list from the folder*/
  // variable initial
  int img_number_left = 0;
  //string argv_1 = "/home/aras/stereo_images/stereo_45cm-baseline/2013-10-25_myStereo-baseline-45cm_cart_outside_forest/seq100/acso_test/frames_pcd_left";
  string argv_1 = "/home/aras/stereo_images/kinect_sugv/2014-02-18_office-mit-karsten";
  ...
  this -> frames_number_total = img_number_left;
  images_idx = 0;

}//constructor

void
mPointCloudProc::keyboardCallback(const pcl::visualization::KeyboardEvent& event, void*)
{
  if (event.keyUp())
  {
...
void mPointCloudProc::processCloud(const CloudConstPtr& cloud)
{
...
void mPointCloudProc::pcd2tImage()
{
  pcl::PCLImage image;
  ...
  FINROC_LOG_PRINT(DEBUG_VERBOSE_1, "IMAGE.WIDTH: ", image.width); //640
  FINROC_LOG_PRINT(DEBUG_VERBOSE_1, image.height, "  ", image.encoding); //480 and RGB8 -> 8*3=24
...
//----------------------------------------------------------------------
// mPointCloudProc destructor
//----------------------------------------------------------------------
mPointCloudProc::~mPointCloudProc()
{}

//----------------------------------------------------------------------
// mPointCloudProc OnStaticParameterChange
//----------------------------------------------------------------------
void mPointCloudProc::OnStaticParameterChange()
{
...
//----------------------------------------------------------------------
// mPointCloudProc OnParameterChange
//----------------------------------------------------------------------
void mPointCloudProc::OnParameterChange()
{
...
//----------------------------------------------------------------------
// mPointCloudProc Update
//----------------------------------------------------------------------
void mPointCloudProc::Update()
{
