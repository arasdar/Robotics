...
void
tStereoProcessing::processCloud(const CloudConstPtr& cloud)
{
  /*Compute the normals*/
  ...
  *label_image = *cloud;
  ...
  /*
    ...
    unsigned int largest = region_indices_size.at(region_indices_size.size() - 1);
  */

  unsigned largest = *std::max_element(region_indices_size.begin(), region_indices_size.end());

  /*
  //  unsigned int threshold_min_dominant_plane_cloud = (cloud->width / 10) * (cloud->height / 10) * 0.05; //(cloud->width / 10) * (cloud->height / 10) * 1.5 * 0.5;  //2500
  ...
  */
  ...
  for (unsigned int i = 0; i < region_indices.size(); i++) //only looking max value
  {
    ...
    if (region_indices[i].indices.size() > 100)
    {
      ...
      {
        /*        //        ground_image->points[region_indices[i].indices[j]].g =
         * static_cast<uint8_t>((cloud->points[region_indices[i].indices[j]].g + 255) / 2);*/
    ...
    /*detecting the dominant segmented plane region*/
    if (region_indices[i].indices.size() == largest)
    {
