...
const double sector_cell_height = 10; //30;
const double sector_polar_cell_angle = 3; //6; //9;
const double seclet_cell_height = 1; //1; //this needs to be fixed and equal among all TODO
...
//    rrlib::math::tVector<2, double, rrlib::math::vector::Polar, rrlib::math::angle::Degree, rrlib::math::angle::Unsigned> low_length = bounds.lower_bounds[1];
    ...
    for (unsigned int length = 0; length <= sector_cell_height; length += seclet_cell_height)
    {
      first_corner.Length() = length; //low_length.Length();
      first_corner[0] = rrlib::math::tAngleDegUnsigned(low_angle[0]);

      second_corner.Length() = (length + seclet_cell_height); //high_length.Length();
      second_corner[0] = rrlib::math::tAngleDegUnsigned(low_angle[0]);

      third_corner.Length() = (length + seclet_cell_height); //high_length.Length();
