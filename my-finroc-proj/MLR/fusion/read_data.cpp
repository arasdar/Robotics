...
std::vector<cv::Mat> tMLR::read_data(/*INPUT*/const char** argv, /*INPUT*/ const eDATA_TYPE& data_type)
{

  // reading all the sensors data separately first and concatenate them all at the end
  std::vector<cv::Mat> vec_I;
              ...
              )
              {
                ...
                if (camera_image.rows/*size().width*/ > 0 && camera_image.cols/*size().height*/ > 0  && !camera_image.empty() /*camera image not empty*/)
                {
                  if (data_type == eDATA_TYPE::eIMAGE_DATA)
                  {
                    ...
                    cv::Mat I_img = cv::imread(camera_image_filepath, CV_LOAD_IMAGE_GRAYSCALE);
                    /*string */window_name = "camera_image grayscale"; //camera_image_filename + "___" + time_stamp_string_label + "___" + cv::format("_%d", data_index);
                    cv::namedWindow(window_name, CV_WINDOW_NORMAL);
                    imshow(window_name, I_img);
                    toNormalize(I_img, I_img, eDATA_TYPE::eIMAGE_DATA); // 0-255 == unsigned int
                    /*string */window_name = "camera_image normalized"; //camera_image_filename + "___" + time_stamp_string_label + "___" + cv::format("_%d", data_index);
                    cv::namedWindow(window_name, CV_WINDOW_NORMAL);
                    imshow(window_name, I_img);
                    cv::waitKey(1);
                    vec_I.push_back(I_img);
                  }
                  ...
                  if (data_type == eDATA_TYPE::eDISTANCE_DATA)
                  {
                    ...
                    cv::Mat I_dist_norm;
                    toNormalize(I_dist, I_dist_norm, eDATA_TYPE::eDISTANCE_DATA); // 0-0.8 meter == double
                    vec_I.push_back(I_dist_norm);
                  }
