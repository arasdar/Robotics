...
void tMLR::recognize_test(/*INPUT*/const Mat& I_all, /*INPUT*/const Mat& /*images*/I_all_img, /*INPUT*/const Mat& I_img_sample, /*INPUT*/const char** argv)
{
...
//  cv::Mat mean_all;
//  read_mean(argv, "mean_all", mean_all);

  cv::Mat mean_loc;
  read_mean(argv, "mean_loc", mean_loc /*same as pose*/);
  ...
  for (/*unsigned*/ int j_I_new = 0; j_I_new < I_all.cols; ++j_I_new) // j, col, x and width
  {
    ...
    for (/*unsigned */int j_I = 0 ; j_I < I_all.cols; j_I++) //pool of patches
    {
      ...
      compare(I, I_new, mean_loc, vec_diff_norms, vec_sim_values);
