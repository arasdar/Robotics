...
void tMLR::recognize(/*INPUT*/const Mat& I_scaled_vec, /*INPUT*/const Mat& I_new_scaled_vec, /*INPUT*/const Mat& I_img_sample, /*INPUT*/const char** argv)
{
  ...
  for (/*unsigned*/ int j_I_new = 0; j_I_new < I_new_scaled_vec.cols; ++j_I_new) // j, col, x and width
  {
    ...
    Mat I_new = I_new_scaled_vec.col(j_I_new).clone();
    cv::Mat I_new_img = I_new.clone().reshape(0 /* ZERO channels or Matrix*/, I_img_sample.rows);
    ...
    for (/*unsigned */int j_I = 0 ; j_I < I_scaled_vec.cols; j_I++) //pool of patches
    {
      // showing the the current database
      Mat I = I_scaled_vec.col(j_I).clone();
      cv::Mat I_img = I.clone().reshape(0, I_img_sample.rows);
    ...
    for (unsigned int best_match_index = 0; /*EMPTY*/ ; best_match_index++)
    {
      ...
      Mat min_diff_result = I_scaled_vec.col(min_index_diff).clone();
      ...
      Mat max_sim_result = I_scaled_vec.col(max_index_sim).clone();
      ...
      Mat min_diff_mu_result = I_scaled_vec.col(min_index_diff_mu).clone();
      ...
      Mat max_sim_mu_result = I_scaled_vec.col(max_index_sim_mu).clone();
      ...
      Mat min_diff_mu_V_result = I_scaled_vec.col(min_index_diff_mu_V).clone();
      ...
      Mat max_sim_mu_V_result = I_scaled_vec.col(max_index_sim_mu_V).clone();
      ...
      Mat min_diff_mu_V_S_result = I_scaled_vec.col(min_index_diff_mu_V_S).clone();
      ...
      Mat max_sim_mu_V_S_result = I_scaled_vec.col(max_index_sim_mu_V_S).clone();
