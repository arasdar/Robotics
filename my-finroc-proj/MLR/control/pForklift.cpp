...
void CreateMainGroup(const std::vector<std::string> &remaining_arguments)
{
  ...
  // reading the input argument which is the dir path for reading the learned files and also the dataset
  if (remaining_arguments.size() != 1)
  {
    FINROC_LOG_PRINT(ERROR, "Exactly the folder to read the written IO files has to be specified");
    exit(1);
  }

  finroc::runtime_construction::tCreateFrameworkElementAction& framework_element_action = finroc::runtime_construction::LoadComponentType("libfinroc_projects_stereo_traversability_experiments_recognition.so", "Recognition");
  finroc::core::tFrameworkElement* framework_element = framework_element_action.CreateModule(main_thread, "Recognition");
  finroc::stereo_traversability_experiments::control::mRecognition* recognition = dynamic_cast<finroc::stereo_traversability_experiments::control::mRecognition*>(framework_element);
  recognition->dir_path.Set(remaining_arguments[0]);
