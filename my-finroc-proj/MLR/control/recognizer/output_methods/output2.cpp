...
void mRecognition::generate_output_test(/*INPUT*/const std::vector<RecordedData>& vec_recorded_data)
{
  ...
  if (std::abs(vec_recorded_data[7]/*sim_mu_V_S_value*/.data_index - vec_recorded_data[3]/*diff_mu_V_S_norm*/.data_index) < 5) // meaning if they were equal
  {
    if (std::abs(vec_recorded_data[6]/*sim_mu_V_value*/.data_index - vec_recorded_data[2]/*diff_mu_V_norm*/.data_index) < 5) // the same
    {
