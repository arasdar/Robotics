...
int CvexStereoCameraCalibration::
getImageCount()
{
	return image_count;
}

void CvexStereoCameraCalibration::
getKRK(CvMat* srcK,CvMat* R, CvMat* destK, CvMat* dest)
{
...
CvexStereoCameraCalibration(CvSize img_size, CvSize _pattern_size,double _CHESS_LENGTH, int _max_chess_count)
{
...
bool CvexStereoCameraCalibration::
findChess(IplImage* left_img,IplImage* right_img)
{
	bool ret=false;
...
void CvexStereoCameraCalibration::
drawChessboardCorners(IplImage* src, IplImage* dest,int left_or_right)
{
...
void CvexStereoCameraCalibration::
rectifyImageRemap(IplImage* src, IplImage* dest, int left_right)
{
	IplImage* _dest;
	if(src ==dest)
	{
...
void CvexStereoCameraCalibration::
getRectificationMatrix(CvMat* h_left, CvMat* h_right)
{
	...
	cvStereoRectify(leftCamera.intrinsic,rightCamera.intrinsic,
		leftCamera.distortion,rightCamera.distortion,image_size,
		relate_rot,relate_trans,R1,R2,P1,P2);
	for(int j=0;j<3;j++)
	{
		for(int i=0;i<3;i++)
		{
	...
	cvInitUndistortRectifyMap(leftCamera.intrinsic, leftCamera.distortion,
		R1, P1,remap_left_x,remap_left_y);

	//cvexShowMatrix(remap_left_x);
	cvInitUndistortRectifyMap(rightCamera.intrinsic, rightCamera.distortion,
...
solveStereoParameter()
{
	...
	for (int i = 0; i < image_count; i++)
	{
		for (int j = 0; j < pattern_size.height; j++) 
		{
			...
			{
				objects[i * PATTERN_SIZE + j * pattern_size.width + k].x = (float)(j * CHESS_LENGTH);
				objects[i * PATTERN_SIZE + j * pattern_size.width + k].y = (float)(k * CHESS_LENGTH);
				objects[i * PATTERN_SIZE + j * pattern_size.width + k].z = 0.0f;
			}
		}
	}		
	...
	for (int i = 0; i < image_count; i++)
	{
		for (int j = 0; j < pattern_size.height; j++) 
		{
			for (int k = 0; k < pattern_size.width; k++) 
			{
				corners1[i * PATTERN_SIZE + j * pattern_size.width + k].x = leftCamera.subpix_corners[i][j * pattern_size.width + k].x;
				corners1[i * PATTERN_SIZE + j * pattern_size.width + k].y = leftCamera.subpix_corners[i][j * pattern_size.width + k].y;
			}
		}
	}
	cvInitMatHeader (&image_points1, image_count*PATTERN_SIZE, 1, CV_32FC2, corners1);
	cvCalibrateCamera2 (&object_points, &image_points1, &point_counts, image_size, leftCamera.intrinsic, leftCamera.distortion);
	...
	for (int i = 0; i < image_count; i++)
	{
		for (int j = 0; j < pattern_size.height; j++) 
		{
			for (int k = 0; k < pattern_size.width; k++) 
			{
				corners2[i * PATTERN_SIZE + j * pattern_size.width + k].x = rightCamera.subpix_corners[i][j * pattern_size.width + k].x;
				corners2[i * PATTERN_SIZE + j * pattern_size.width + k].y = rightCamera.subpix_corners[i][j * pattern_size.width + k].y;

			}
		}
	}
	cvInitMatHeader (&image_points2, image_count*PATTERN_SIZE, 1, CV_32FC2, corners2);
	cvCalibrateCamera2 (&object_points, &image_points2, &point_counts, image_size, rightCamera.intrinsic, rightCamera.distortion);

	cvStereoCalibrate(&object_points,&image_points1,&image_points2,&point_counts,
