...
public:
	CvexStereoCameraCalibration(CvSize img_size, CvSize _pattern_size,double _CHESS_LENGTH, int _max_chess_count=1000);
	~CvexStereoCameraCalibration();

	bool findChess(IplImage* left_img,IplImage* right_img);
	void drawChessboardCorners(IplImage* src, IplImage* dest,int left_or_right=CVEX_STEREO_CALIB_LEFT);
	...
	void getRectificationMatrix(CvMat* h_left, CvMat* h_right);
