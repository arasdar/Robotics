...
public:
	...
	void init(CvSize img_size, CvSize _pattern_size,double _CHESS_LENGTH, int _max_chess_count=10000);
	int getImageCount();

	CvexCameraCalibration();
	CvexCameraCalibration(CvSize img_size, CvSize _pattern_size,double _CHESS_LENGTH, int _max_chess_count=10000);
	~CvexCameraCalibration();

	bool findChessboardCorners(IplImage* src_img, bool isStore=true);
