...
IplImage* cvexConnect(IplImage* src1, IplImage* src2, int mode)
{
  ...
  default:
    ret = cvCreateImage(cvSize(src1->width + src2->width, MAX(src1->height, src2->height)), 8, 3);
  ...
  case CVEX_CONNECT_VERTICAL:
    ret = cvCreateImage(cvSize(MAX(src1->width, src2->width), src1->height + src2->height), 8, 3);
    cvSetImageROI(ret, cvRect(0, 0, src1->width, src1->height));
    cvCopy(src1, ret);
...
IplImage* cvexConnectMulti(IplImage** src, int arrayWidth, int arrayHeight, unsigned char* _mask)
{
  IplImage* ret;
  ...
  ret = cvCreateImage(cvSize(src[0]->width * arrayWidth, src[0]->height * arrayHeight), 8, 3);
  for (int j = 0; j < arrayHeight; j++)
  {
    for (int i = 0; i < arrayWidth; i++)
    {
      ...
      {
        cvSetImageROI(ret, cvRect(src[0]->width * i, src[0]->height * j, src[0]->width, src[0]->height));
...
void cvexPutText(IplImage* render, char* text, CvPoint orign, CvScalar color, double amp, double shear, int fontType, int thickness)
{
  CvFont font;
