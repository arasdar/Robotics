...
void tStereoProcessing::run_initialize()
{
  ...
  */
  ...
  cb_args.viewerPtr_3 = pcl::visualization::PCLVisualizer::Ptr(viewer_proc_segm_normal);
  cb_args.viewerPtr_4 = pcl::visualization::PCLVisualizer::Ptr(viewer_proc_segm_texture);
  ...
  viewer_proc_segm_normal ->removeAllPointClouds();
  viewer_proc_segm_texture ->removeAllPointClouds();
  text_id = 0;
  for (int i = 0; i < 100; i++)
  {
    viewer->removeText3D(text_id_str[i]);
    viewer_disparity->removeText3D(text_id_str[i]);
    viewer_proc_segm_normal->removeText3D(text_id_str[i]);
    viewer_proc_segm_texture->removeText3D(text_id_str[i]);
  }//for
