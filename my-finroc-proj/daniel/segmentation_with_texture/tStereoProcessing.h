 ...
 */
//----------------------------------------------------------------------
#ifndef __projects__stereo_traversability_experiments__daniel__segmentation_with_texture__tStereoProcessing_h__
#define __projects__stereo_traversability_experiments__daniel__segmentation_with_texture__tStereoProcessing_h__
...
#include "projects/stereo_traversability_experiments/daniel/segmentation_with_texture/libsegmentation/tPlaneTextureComparator.h"
#include "projects/stereo_traversability_experiments/daniel/segmentation_with_texture/libsegmentation/tGroundPlaneComparator.h"
#include "projects/stereo_traversability_experiments/daniel/segmentation_with_texture/libsegmentation/tOrganizedConnectedComponentSegmentation.h"
...
typedef HaralickPointType Texture;
...
public:
  ...
  void stereo_rectify(const string input_images_left, const string input_images_right);
  void stereo_cropping(Mat* img_rect, const Rect* validRoi);
  void stereo_reconst();

  void processImage_texture();
...
private:
  ...
  boost::shared_ptr<visualization::PCLVisualizer> viewer_proc_segm_normal;
  boost::shared_ptr<visualization::PCLVisualizer> viewer_proc_segm_texture;

  boost::shared_ptr<visualization::ImageViewer> image_viewer;
  boost::shared_ptr<visualization::ImageViewer> image_viewer_disparity;
  boost::shared_ptr<visualization::ImageViewer> image_viewer_proc_segm_normal;
  boost::shared_ptr<visualization::ImageViewer> image_viewer_proc_segm_texture;
  ...
  aras::libstereo::tTestACSO stereo;
  int smooth_weak;
  int smooth_strong;
  PointCloud<RGB>::Ptr prev_img_disp;
  CloudPtr prev_cloud_disp;

  aras::libfeature::tIntegralImageNormalEstimation<PointT, pcl::Normal> ne;
  PointCloud<Normal>::ConstPtr prev_normal_cloud;
  PointCloud<Texture>::ConstPtr prev_texture_cloud;
  CloudPtr prev_cloud;

  libsegmentation::PlaneTextureComparator<PointT, Texture>::Ptr  comp_texture;
  libsegmentation::GroundPlaneComparator<PointT, Normal>::Ptr  comp_normal;
  libsegmentation::OrganizedConnectedComponentSegmentation<PointT, Label> segm_texture;
  libsegmentation::OrganizedConnectedComponentSegmentation<PointT, Label> segm_normal;
  CloudPtr prev_cloud_segm_normal, prev_cloud_segm_texture;
  PointCloud<pcl::Label> prev_cloud_segment_labels_normal, prev_cloud_segment_labels_texture;
  vector<pcl::PointIndices> prev_cloud_segments_normal, prev_cloud_segments_texture;
  unsigned int thresh_segments = 100;
};
