...
void tStereoProcessing::processCloud_trav_segm2plane_appear()
{
  std::vector<PointIndices> segment_indices(prev_cloud_segments_appear);
  ...
  *cloud = *prev_cloud;
  ...
  prev_cloud_planes_appear.clear();
  prev_cloud_planes_appear.resize(segment_indices.size());

  prev_cloud_planes_cent_pt_appear.clear();
  prev_cloud_planes_cent_pt_appear.resize(segment_indices.size());

  for (unsigned int i = 0; i < segment_indices.size(); i++)
  {

    prev_cloud_planes_appear[i] = Eigen::Vector4f(0, 0, 0, 0);

    if (True) // try each segment; replace by test over number of valid points; //segment_indices[i].indices.size() > thresh_segments)
    {
      ...
      prev_cloud_planes_appear[i] = plane_params;
      prev_cloud_planes_cent_pt_appear[i] = np1;
