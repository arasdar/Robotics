...
class StereoVisionProcessing
{
  ...
  public:
	...
	int stereo_capture_ravon(int i)
	{
        ...
        switch (key)
        {
				...
				case ' ': //Save an image
				  ...
				  imwrite(filename, frame_found_0);
	...
	int stereo_rectify ( const Mat png_images_left, const Mat png_images_right )
	{
		...
		Mat img1 ;
		Mat img2 ;
		png_images_left.copyTo(img1);
		png_images_right.copyTo(img2);
		...
		//display the results
		namedWindow("left_1", 0);
		imshow("left_1", left_rect_png_img);
		namedWindow("right_1", 0);
		imshow("right_1", right_rect_png_img);

		return 0;
	}// stereo_rectify

	int stereo_rectify ( const string input_png_images_left, const string input_png_images_right )
	{
			...
			namedWindow("left_img1", 0);
			imshow("left_img1", img1);
			namedWindow("right_img2", 0);
			imshow("right_img2", img2);
			...
			{
				namedWindow("left", 0);
				imshow("left", left_rect_png_img);
				namedWindow("right", 0);
				imshow("right", right_rect_png_img);
			}
...
//				  		  cloud (x, image.rows - y - 1) = color;
    ...
    PointCloud<RGB>::Ptr png2pcd (const string input_png_images)
    {
		...
		int color_mode = 0; //grayscale
		Mat image = imread( img1_filename, color_mode );
...
main (int argc, char** argv)
{
  ...
  // Process and display
  StereoVisionProcessing hrcs (left_images, right_images, img_pairs_num, input_intrinsic_filename, input_extrinsic_filename);
//  hrcs.run ();


  // ravon capturing - awesome!!
  for(int i=1;;i++){
	  	  hrcs.stereo_capture_ravon(i);
  }// for
