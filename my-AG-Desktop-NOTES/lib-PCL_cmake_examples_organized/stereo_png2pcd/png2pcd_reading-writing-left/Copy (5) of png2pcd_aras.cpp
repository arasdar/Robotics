  ...
  for (;;)
  {
	...
	//  std::vector<int> png_file_indices = parse_file_extension_argument (argc, argv, ".png");
	  ...
	  if (mode.compare ("FORCE_COLOR") == 0)
	  {
	   ...
	   std::cerr << "output pcd file " << ss.str () << std::endl;


//		std::stringstream ss2;
//		ss2 << dir_name_ << "/" << file_name_ << "_savecloud_" << img_index << ".pcd";
//	   std::cerr << "output pcd file " << ss2.str () << std::endl;
//		// Save the point cloud into a PCD file
//		saveCloud<RGB> (ss2.str (), cloud, format);
	  }
