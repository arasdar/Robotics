...
main (int argc, char** argv)
{
  ...
  for (;;)
  {
	  ...
	  reader->SetFileName ( (char*) left_images[img_index].c_str());
	...
	cout << "output pcd file from writer_.writeBinary<RGB> " << ss.str () << std::endl;

	// save ouput pcd file savePCD
	stringstream ss2;
	ss2 << dir_name_ << "/" << file_name_ << "_savePCDFileASCII_" << img_index << ".pcd";
	pcl::io::savePCDFileASCII (ss2.str(), cloud);
	cout << "output pcd file from savePCDFileASCII " << ss2.str() << endl;
